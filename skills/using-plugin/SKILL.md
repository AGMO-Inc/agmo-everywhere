...
| `agmo:architect` | Analyze, verify, debug | **fable (고정)** | Verification, debugging, impact analysis |
...
| `agmo:frontend` | Frontend quality gate — visual, accessibility, responsive | **fable (고정)** | Figma vs browser comparison, WCAG review, responsive verification |
| `agmo:android-specialist` | Android frontend quality gate — visual, accessibility, responsive (Compose) | **fable (고정)** | Figma vs Compose Preview comparison, Android WCAG review, responsive verification |

## Category Routing

executor/explore/archivist를 dispatch할 때는 `references/category-routing.md`의 모델 라우팅 표를 읽어라. 요약: **planner·architect·critic·frontend·android-specialist = fable 고정(model 파라미터 전달 금지)**, **executor·explore·archivist = 카테고리 라우팅(haiku/sonnet/fable 명시 전달)**.
...
4. **Token efficiency.** planner/architect/critic은 항상 fable. executor/explore/archivist는 카테고리 라우팅 (haiku/sonnet/fable).
